...
function NoCohort({ isAdmin }: { isAdmin: boolean }) {
  return (
    <div className="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
      <h1 className="font-display text-2xl tracking-tight text-navy mb-3">
      ...
      <p className="text-stone-600 leading-relaxed mb-8">
        ...
        <a href="mailto:nate@cappawork.com" className="text-gold hover:text-gold/80 underline">
          nate@cappawork.com
        </a>{" "}
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <Link
          ...
          className="rounded-xl border border-stone-200 bg-white p-5 text-left hover:border-gold/40 hover:shadow-sm transition-all group"
        >
          <div className="flex items-center justify-between">
            <span className="font-medium text-stone-900">AI for Business Leaders</span>
            <ArrowRight size={16} className="text-stone-400 group-hover:text-gold transition-colors" />
          </div>
      ...
      {isAdmin && (
        <p className="mt-8 text-sm text-stone-500">
          You're an admin —{" "}
          <Link href="/admin/cohorts" className="text-gold hover:text-gold/80 underline">
            manage cohorts
          </Link>{" "}
...
function Chooser({ participations }: { participations: ParticipationWithCohort[] }) {
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <h1 className="font-display text-2xl tracking-tight text-navy mb-2">Your cohorts</h1>
      ...
      <div className="space-y-4">
        {participations.map((p) => (
          <Link
            ...
            className="flex items-center justify-between rounded-2xl border border-stone-200 bg-white p-6 hover:border-gold/40 hover:shadow-sm transition-all group"
          >
            <div>
            ...
            <ArrowRight size={20} className="text-stone-400 group-hover:text-gold group-hover:translate-x-1 transition-all" />
          </Link>
        ))}
      </div>
    </div>
